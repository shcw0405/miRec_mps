...
def get_parser():
    ...
    parser.add_argument('--device', type=str, default='mps') # None -> cpu
    ...
    # For REMI
    parser.add_argument('--rbeta', type=float, default=4)
...
class DataIterator_with_time(torch.utils.data.IterableDataset):
...
class DataIterator_without_time(torch.utils.data.IterableDataset):
    ...
    def __next__(self):
        ...
        # 返回用户列表（batch_size）、物品列表（label）（batch_size）、
        # 历史物品列表（batch_size，seq_len）、历史物品的mask列表（batch_size，seq_len）
#        print(np.array(user_id_list).shape)
        return user_id_list, item_id_list, hist_item_list, hist_mask_list, (time_matrix_list, adj_matrix_list)

    
def get_DataLoader(source, batch_size, seq_len, train_flag=1, args=None):
    if args.model_type in ['PIMI']:
        dataIterator = DataIterator_with_time(source, batch_size, seq_len, train_flag)
    else:
        dataIterator = DataIterator_without_time(source, batch_size, seq_len, train_flag)
    return DataLoader(dataIterator, batch_size=None, batch_sampler=None)#, num_workers=10
...
# 获取模型
def get_model(dataset, model_type, item_count, user_count, batch_size, hidden_size, interest_num, seq_len, routing_times=3, args=None, device=None):
    add_pos = True
    if args:
        add_pos = args.add_pos == 1
    if model_type == 'DNN': 
        model = DNN(item_count, user_count, hidden_size, batch_size, seq_len)
    elif model_type == 'GRU4Rec': 
        model = GRU4Rec(item_count, user_count, hidden_size, batch_size, seq_len, num_layers=args.layers, dropout=args.dropout)
    elif model_type == 'MIND':
        relu_layer = True if dataset == 'book' else False
        model = MIND(item_count, user_count, hidden_size, batch_size, interest_num, seq_len, routing_times=routing_times, relu_layer=relu_layer)
    elif model_type == 'ComiRec-DR':
        relu_layer = False
        hard_readout = False if dataset == 'kindle' else True
        model = ComiRec_DR(item_count, user_count, hidden_size, batch_size, interest_num, seq_len, routing_times=routing_times, relu_layer=relu_layer, hard_readout=hard_readout)
    elif model_type == 'ComiRec-SA':
        model = ComiRec_SA(item_count, user_count, hidden_size, batch_size, interest_num, seq_len, add_pos=add_pos, args = args, device = device)
    elif model_type == 'REMI':
#        print(hidden_size)
        model = REMI(item_count, user_count, hidden_size, batch_size, interest_num, seq_len, add_pos=add_pos, beta=args.rbeta, args=args, device=device)
    elif model_type == 'Re4':
        model = Re4(item_count, user_count, hidden_size, batch_size, interest_num, seq_len, device=device, args=args)
    elif model_type == 'PAMI':
        model = PAMI(item_count, user_count, hidden_size, batch_size, interest_num, seq_len, add_pos=add_pos, args=args, device=device)
    elif model_type == 'DropoutComiRec':
        model = DropoutComiRec(item_count, user_count, hidden_size, batch_size, interest_num, seq_len, add_pos=add_pos, args=args, device=device)
    elif model_type == 'REMIuseremb':
        model = REMIuseremb(item_count, user_count, hidden_size, batch_size, interest_num, seq_len, add_pos=add_pos, args=args, device=device)
    else:
