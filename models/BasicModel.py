...
class NCELoss(nn.Module):
    ...
    def forward(self, target, input, embs, interests, loss_fn = None, *args, **kwargs):
...
#           在noise概率分布中，抽到noise/target的概率值
            logit_target_in_model, logit_noise_in_model = self._get_logit(target, noise_samples, input, embs, interests, *args, **kwargs)#######修改了 多了一个interests
#            print(input.shape)
#            print(logit_target_in_model.shape)
#            print(logit_noise_in_model.shape)
#            print(logit_noise_in_noise.shape)
#            print(logit_target_in_noise.shape)
#            print(logit_target_in_model)
#            print(logit_noise_in_model)
#            print(logit_noise_in_noise) 即使是REMI，这里的输出都是一样的
#            print(logit_target_in_noise) 即使是REMI，这里的输出都是一样的
#            return
    ...
    def _get_logit(self, target_idx, noise_idx, input, embs, interests, *args, **kwargs): #修改了 多了一个interests
        ...
        target_logit, noise_logit = self.get_score(target_idx, noise_idx, input, embs, interests, *args, **kwargs)
    ...
    def get_score(self, target_idx, noise_idx, input, embs, interests, *args, **kwargs):
        ...
        """
#        target_batch = embs[target_idx].squeeze(1)
#        print(embs[target_idx].shape) #[B,1,64]
#        print(embs[target_idx].squeeze(1).shape) #[B,64]
#        print()
#        product = (interests * target_batch.unsqueeze(1).expand(-1,interests.shape[1],-1)).sum(dim=-1)
#        product = torch.matmul(interests, target_batch.unsqueeze(-1)).squeeze(-1)
#        print(product.shape)
#        max_values, _ = product.max(dim=1)
#        output = max_values.unsqueeze(1)
#        noise_batch = embs[noise_idx.squeeze(1)]
#        print(noise_idx.shape)
#        print(noise_batch.shape)
#        print(interests.shape)
#        print(noise_batch.shape)
#        print(noise_batch.transpose(1,2).shape)
#        interests_expanded = interests.unsqueeze(2).expand(-1,-1,noise_idx.shape[-1],-1)
#        product_noise = (interests_expanded * noise_batch.unsqueeze(1)).sum(dim=-1)
#        product_noise = torch.matmul(interests, noise_batch.transpose(1,2))
#        print(product_noise.shape)
#        min_values= product_noise.mean(dim=1) #min max mean
#        output_noise = min_values.unsqueeze(1)
#        print(min_values.shape)
#        print(target_idx.shape)  #[B,1]
#        print(noise_idx.shape)
#        print(input.shape)
#        print(embs.shape)
        ...
        noise_score = torch.matmul(
            interests, noise_batch.t()
        ) 
#        print(noise_score.shape) # [B, 4, 1280]
#        print(target_score.view(original_size).shape)  # [B,1]
#        print(noise_score.view(*original_size, -1).shape) # [B, 1, 5120]
#        print(noise_score.max(dim=1)[0].view(*original_size, -1).shape) # [B, 1, 1280]
        return target_score.view(original_size), noise_score.max(dim=1)[0].view(*original_size, -1)
...
def build_noise(number, args=None):
    ...
    if args.sample_prob == 0:
        return build_uniform_noise(number, device=device)
    if args.sample_prob == 1:
        return build_log_noise(number, device=device)

def build_log_noise(number, device=None):
    total = number
    freq = torch.Tensor([1.0] * number).to(device)
    noise = freq / total
...
def build_uniform_noise(number, device=None):
    total = number
    freq = torch.Tensor([1.0] * number).to(device)
    noise = freq / total 
    assert abs(noise.sum() - 1) < 0.001
...
class BasicModel(nn.Module):
    ...
    def __init__(self, item_num, user_num, hidden_size, batch_size, seq_len=50, beta=0):
        ...
        self.user_embeddings = nn.Embedding(self.user_num, self.hidden_size, padding_idx=0)
        self.name = 'base'
        self.beta = beta
        self.interest_num = 0
    ...
    def print_parameters(self):#REMI没有使用
        print("*"*99)
        print("Parameters:")
...
#        print(pos_items.unsqueeze(-1).shape)
#        print(readout.shape)
#        print(type(self.item_embeddings))
        return self.sample_loss(pos_items.unsqueeze(-1), readout, self.item_embeddings.weight, interests)

    def output_users(self, data_loader, device):
        """获取所有用户的embedding
        Returns:
            user_embeddings: dict, key为user_id, value为user_embedding
        """
        self.eval()
        user_embeddings = {}
        
        with torch.no_grad():
            for user_ids, _, item_list, mask, times in data_loader:
                item_eb = self.item_embeddings(torch.LongTensor(item_list).to(device))
                mask = torch.FloatTensor(mask).to(device)
                
                # 获取用户embedding
                if self.name in ['ComiRec_DR', 'MIND']:
                    # 多兴趣模型
                    user_eb, _ = self.forward(torch.LongTensor(item_list).to(device), 
                                           None, mask, times, device, train=False)
                elif self.name in ['ComiRec_SA', 'REMI', 'Re4', 'PAMI']:
                    # 多兴趣模型
                    user_eb, _, _, _, _ = self.forward(torch.LongTensor(item_list).to(device), 
                                                     None, mask, times, device, train=False)
                else:
                    # 单兴趣模型
                    user_eb, _ = self.forward(torch.LongTensor(item_list).to(device), 
                                           None, mask, device, train=False)
                
                # 保存用户embedding
                for idx, user_id in enumerate(user_ids):
                    user_embeddings[user_id] = user_eb[idx].cpu().numpy()
